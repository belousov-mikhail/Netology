...
class TranslateIt(object):
    ...
    def _set_output(self, output):
        path, file = os.path.split(self.source_name)
        if output=='same':
            self.output_name = os.path.join(path,'translated_' + self.params['lang'] + '_' + file)
        else:
...
def create_parser():
    ...
    parser.add_argument('-o', '--output', default='same')
    parser.add_argument('-f', '--froml', default='auto')
    parser.add_argument('-t', '--tol', default='ru')
    return parser
...
if __name__ == '__main__':
    ...
    a = TranslateIt()
    a.translate_it(namespace.source, namespace.output, namespace.froml, namespace.tol)
