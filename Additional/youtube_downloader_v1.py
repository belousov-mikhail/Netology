...
#TBD
# - use regex to validate names (smb_to_avoid = "0123456789#<$+%>!`&*'|{?\"=}/:\ @")
...
# 3. Can I check results of downloading in a more efficient way? E.g. can I get file size for Chrome before download starts?
...
def downloadFromSavemedia(url, driver, clip_link, number):
    ...
    try:
        ...
        if time.time() - start_time > 120:
            raise Exception('Overtime')  # try to handel vpn connection quality issues
        return True
    except:
        print('smth went wrong for file #{}'.format(number))
        return False
...
def main():
    ...
    playlist = 'https://www.youtube.com/playlist?list=PLQ176FUIyIUY5Ii58pzoZhS_3qIBL80nz'
