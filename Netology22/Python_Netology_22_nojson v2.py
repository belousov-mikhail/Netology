...
def detect_encoding(news_entry):
    rawdata =open(news_entry, "rb").read()
    open(news_entry).close()
    result = chardet.detect(rawdata)
    return result['encoding']

def main():
    home_dir = os.getcwd() #by default files are in the home directory
    news_files = []
    ...
    for news_entry in news_files:
        charset = detect_encoding(news_entry)
